...
for col in range(0,8):
    for ro in range(1,7):
        button_list.append(tk.Button(frame, width=8, height=3, bg='black', fg='white', font=('Helvetica', 20, 'bold'), bd=4, text=tags[i]))

        button_list[i].grid(row=ro, column=col, pady=1, padx=1)
        i+=1
