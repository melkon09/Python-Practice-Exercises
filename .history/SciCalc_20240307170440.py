...
for ro in range(6,10):
    for col in range(0,5):
        if tags_func[i]=='C' or tags_func[i]=='AC':
            button_list.append(tk.Button(frame, width=2, height=1, bg='red', fg='white', font=('Helvetica', 10, 'bold'), bd=4, text=tags_func[i]))
            button_list[i].grid(row=ro, column=col)
        elif tags_simple[i]=='='".git\    
        else:
            button_list.append(tk.Button(frame, width=2, height=1, bg='black', fg='white', font=('Helvetica', 10, 'bold'), bd=4, text=tags_func[i]))
            button_list[i].grid(row=ro, column=col)
        i+=1
