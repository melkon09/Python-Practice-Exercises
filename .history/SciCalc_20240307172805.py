...
for ro in range(1,6):
    for col in range(0,5):
        if tags_func[i]=='M+' or tags_func[i]=='MC' or tags_func[i]=='MR':
            ...
            button_list[i].grid(row=ro, column=col, pady=)    
        else:
...
for ro in range(6,9):
    for col in range(0,5):
        if tags_simple[i]=='C' or tags_simple[i]=='AC':
            ...
            button_list.append(tk.Button(frame, width=5, height=2, bg='black', fg='white', font=('Helvetica', 10, 'bold'), bd=4, text=tags_simple[i]))
            button_list[i+25].grid(row=ro, column=col)
        i+=1
