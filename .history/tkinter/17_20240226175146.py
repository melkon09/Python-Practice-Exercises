...
def show_tooltip(event, tooltip, tooltip_text):
    tooltip.geometry(f"+{event.x_parent - 10}+{event.y - 10}") # adjust tooltip position
...
button.bind("<Motion>", lambda event :show_tooltip(event, tooltip, "This is a button."))
button.bind("<Leave>", hide_tooltip)

# Create a label with a tooltip
label=tk.Label(parent, text="Label with a tooltip")
label.pack(padx=10, pady=10)
label.bind("<Motion>", lambda event :show_tooltip(event, tooltip, "This is a label."))
